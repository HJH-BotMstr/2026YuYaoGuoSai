...
| 现象 | 排查 |
|---|---|
| 编排器卡在"等待机械狗进入自动模式" | lite3_driver 未启动或 `/leg_odom2` 无数据：`ros2 topic hz /leg_odom2` |
| 卡在"等待 STANDBY" | grasp_task 未启动/硬件初始化失败，看 grasp_task 日志；`/grasp/state` 有 1Hz 心跳，晚启动也能赶上 |
| AprilTag 对齐超时 | apriltag 子进程日志里看失败原因（Tag 不在视野/光照/相机被占），超时自动取消并 ERROR |
