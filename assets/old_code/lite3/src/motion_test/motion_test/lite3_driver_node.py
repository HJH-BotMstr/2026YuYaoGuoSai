"""Lite3 ROS2 driver node with closed-loop odometry feedback."""
import rclpy
...
from motion_test.transforms import quaternion_from_yaw
...
class Lite3DriverNode(Node):
    def __init__(self):
        ...
        self.manual_override_timeout = self.get_parameter('manual_override_timeout').value
        self.odom_frame_id = self.get_parameter('odom_frame_id').value
        self.base_frame_id = self.get_parameter('base_frame_id').value

        # UDP socket: same port for send and receive on Lite3 protocol.
    ...
    def _select_velocity(self):
        ...
        if manual_dt < self.manual_override_timeout:
            return self.manual_vel

        if auto_dt < self.cmd_timeout:
            return self.auto_vel
    ...
    def send_simple(self, code, value=0):
        self.sock.sendto(struct.pack('<IiI', code, value, 0),
                         (self.robot_ip, self.robot_port))

    def send_complex(self, code, double_val):
        self.sock.sendto(struct.pack('<IIId', code, 8, 1, double_val),
                         (self.robot_ip, self.robot_port))
    ...
    def send_velocity_loop(self):
        self.out_vel = self._select_velocity()
    ...
    def publish_status(self):
        ...
        msg = String()
        msg.data = f'source={source} vx={self.out_vel.linear.x:.3f} vy={self.out_vel.linear.y:.3f} w={self.out_vel.angular.z:.3f}'
